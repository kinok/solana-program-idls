...
func (obj CompressedBinDepositAmount2) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `BinId`:
	err = encoder.Encode(obj.BinId)
	if err != nil {
		return errors.NewField("BinId", err)
...
func (obj CompressedBinDepositAmount2) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding CompressedBinDepositAmount2: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *CompressedBinDepositAmount2) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `BinId`:
	err = decoder.Decode(&obj.BinId)
	if err != nil {
		return errors.NewField("BinId", err)
...
func (obj *CompressedBinDepositAmount2) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling CompressedBinDepositAmount2: %w", err)
	}
	return nil
}

func UnmarshalCompressedBinDepositAmount2(buf []byte) (*CompressedBinDepositAmount2, error) {
	obj := new(CompressedBinDepositAmount2)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
...
type Observation struct {
	...
	CumulativeActiveBinId binary.Int128 `json:"cumulativeActiveBinId"`
...
func (obj Observation) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `CumulativeActiveBinId`:
	err = encoder.Encode(obj.CumulativeActiveBinId)
	if err != nil {
		return errors.NewField("CumulativeActiveBinId", err)
	}
	// Serialize `CreatedAt`:
...
// Layout version
type LayoutVersion binary.BorshEnum

const (
	LayoutVersion_V0 LayoutVersion = iota
	LayoutVersion_V1
)

func (value LayoutVersion) String() string {
	switch value {
	case LayoutVersion_V0:
		return "V0"
	case LayoutVersion_V1:
		return "V1"
	default:
		return ""
	}
...
const (
	AccountsType_TransferHookX AccountsType = iota
	AccountsType_TransferHookY
